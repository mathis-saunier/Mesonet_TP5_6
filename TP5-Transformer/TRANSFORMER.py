...
class Transformer(nn.Module):
    ...
    def forward(self, x,y,y_output_mask,src_key_padding_mask,tgt_key_padding_mask):
        ...
        x = self.positional_encoding_layer(x) # "1D" (prof)
...
class CNNTransformer(nn.Module):
    def __init__(self, config, device):
        ...
        # 🔹 CNN encoder (sequence-preserving)
        ...
        self.y_embedding = nn.Embedding(self.num_classes, self.hidden_size)
    ...
    def forward(self, x, y, y_output_mask, src_key_padding_mask, tgt_key_padding_mask):
        ...
        # 🔹 CNN encoding (keeps T unchanged)
        x = x.transpose(1, 2)            # (B, F, T)
        x = self.cnn(x)                  # (B, hidden_size, T)
